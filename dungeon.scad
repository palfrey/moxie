...
module dungeon(grid_width = 10, grid_length = 10) {
    ...
    difference() {
        ...
        for(n = [1 : grid_length-1]) {
            translate([(tile_width+grid_spacing)*(n-1)+tile_width, -fudge, (tile_depth/2)]) {
                cube(size=[grid_spacing, total_grid_width+(2*fudge), (tile_depth/2)+fudge]);
            }
        }
        for(n = [1 : grid_width-1]) {
            translate([-fudge, (tile_width+grid_spacing)*(n-1)+tile_width, (tile_depth/2)]) {
                cube(size=[total_grid_length+(2*fudge), grid_spacing, (tile_depth/2)+fudge]);
            }
        }
    }
}
